...
export class NoteListComponent {
  ...
  private readonly _orderKey = computed(
    () => `notes_order_${this._authService.user()?.uid ?? 'anon'}`,
  );

  private readonly _orderMap = signal<Record<string, number[]>>(
    JSON.parse(
      localStorage.getItem(`notes_order_${this._authService.user()?.uid ?? 'anon'}`) ?? '{}',
  ...
  protected readonly pinnedNotes = computed(() =>
    this.selectedSection() ? [] : this._filtered().filter((n) => n.pinned),
  );
