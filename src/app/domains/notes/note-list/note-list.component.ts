...
@Component({
  ...
  imports: [NoteCardComponent, MatButtonModule, MatIconModule, TranslatePipe, CdkDropList, CdkDrag],
...
export class NoteListComponent {
  ...
  private readonly _filterService = inject(NoteFilterService);
  ...
  private readonly _orderMap = signal<Record<string, number[]>>(JSON.parse(localStorage.getItem(`notes_order_${this._authService.user()?.uid ?? 'anon'}`) ?? '{}') as Record<string, number[]>);

  private readonly _filtered = computed(() => {
    const { query, dateFrom, dateTo } = this._filterService.filter();
  ...
  protected readonly pinnedNotes = computed(() => (this.selectedSection() ? [] : this._filtered().filter(n => n.pinned)));
  ...
  protected openPreviewNote(note: Note) {
    this._dialog
      .open(NotePreviewComponent, { data: note, width: '600px', maxWidth: '95vw', maxHeight: '90dvh' })
      .afterClosed()
