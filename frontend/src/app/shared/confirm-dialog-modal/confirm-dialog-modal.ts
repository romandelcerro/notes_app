...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <p style="overflow-wrap: break-word; word-break: break-word; margin: 0;">
        {{ data.message }}
      </p>
    </mat-dialog-content>
    <mat-dialog-actions align="end">
      ...
      <button
        mat-flat-button
        [style.--mat-button-filled-container-color]="
          data.confirmVariant === 'warn' ? 'var(--mat-sys-error)' : 'var(--mat-sys-primary)'
        "
        [style.--mat-button-filled-label-text-color]="
          data.confirmVariant === 'warn' ? 'var(--mat-sys-on-error)' : 'var(--mat-sys-on-primary)'
        "
        (click)="confirm()"
      >
        {{ data.confirmLabel }}
...
export class ConfirmDialogModal {
  protected readonly data = inject<{
    ...
    cancelLabel: string;
    confirmLabel: string;
    confirmVariant: 'warn' | 'primary';
  }>(MAT_DIALOG_DATA);
  private readonly _dialogRef = inject(MatDialogRef<ConfirmDialogModal>);
